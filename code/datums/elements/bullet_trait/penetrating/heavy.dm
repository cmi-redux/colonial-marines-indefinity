...
/datum/element/bullet_trait_penetrating/heavy/handle_passthrough_movables(obj/item/projectile/proj, atom/movable/hit_movable, did_hit)
	if(did_hit)
		var/slow_mult = 1
		...
		proj.distance_travelled += (distance_loss_per_hit * slow_mult)
	...
	if(is_type_in_typecache(hit_movable, bullet_destroy_structures))
		...
		proj.damage -= damage_lost_per_pen

	return COMPONENT_BULLET_PASS_THROUGH

/datum/element/bullet_trait_penetrating/heavy/handle_passthrough_turf(obj/item/projectile/proj, turf/closed/wall/hit_wall)
