...
/mob/proc/track_death_calculations()
	if(statistic_exempt || statistic_tracked || !client || !client.player_data)
		return FALSE
...
/mob/proc/count_statistic_stat(statistic_name, amount = 1, weapon)
	if(statistic_exempt || !client.player_data || !faction || !statistic_name)
		return FALSE
	return TRUE
...
/mob/proc/track_friendly_fire(weapon, amount = 1, statistic_name = STATISTICS_FF_SHOT_HIT)
	if(statistic_exempt || !client || !ishuman(src) || !client.player_data)
		return
...
/mob/living/carbon/xenomorph/proc/track_slashes(caste, amount = 1, statistic_name = STATISTICS_SLASH)
	if(statistic_exempt || !client || !client.player_data)
		return
