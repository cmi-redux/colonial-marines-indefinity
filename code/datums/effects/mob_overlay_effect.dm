/datum/effects/mob_overlay_effect //should make this generic and the water_overlay a seperate thing
	...
	var/will_alter_layerings = TRUE
	var/alter_layerings = FALSE
...
/datum/effects/mob_overlay_effect/proc/update_icons(turf/open/input_openturf)
	...
	if(ishuman(affected_atom))
		...
		var/mob/living/carbon/human/affected_human = affected_atom
		var/old_alter_layerings = alter_layerings
		if(will_alter_layerings)
			alter_layerings = abs(pixel_y_offset) >= 8 ? TRUE : FALSE
		affected_human.update_effects(alter_layerings ? ABOVE_MOB_LAYER : FALSE)

		if(old_alter_layerings && !alter_layerings)
			affected_human.layer = initial(affected_human.layer)
			affected_human.plane = initial(affected_human.plane)
			for(var/obj/found_obj in affected_human.vis_contents)
				found_obj.layer = initial(found_obj.layer)
...
/obj/effect/mob_overlay_effect/proc/adjust_transform(turf/open/input_openturf, mob/living/carbon/human/input_human, pixel_y_offset = 0)
	set_up_icon(input_openturf, input_human, pixel_y_offset)

/obj/effect/mob_overlay_effect/proc/set_up_icon(turf/open/input_openturf, mob/living/carbon/human/input_human, pixel_y_offset = 0)
	...
	var/icon/subtraction_texture = icon('icons/effects/mob_overlay_effects.dmi', "culling_mask")
