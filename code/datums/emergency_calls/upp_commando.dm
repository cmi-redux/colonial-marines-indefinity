...
/datum/emergency_call/upp_commando/create_member(datum/mind/mind, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(mob.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(mob.client, JOB_SQUAD_LEADER, time_required_for_job))    //First one spawned is always the leader.
		leader = mob
		arm_equipment(mob, /datum/equipment_preset/upp/commando/leader, TRUE, TRUE)
		to_chat(mob, SPAN_ROLE_HEADER("You are a Commando Team Leader of the Union of Progressive People, a powerful socialist state that rivals the United Americas!"))
	else if(medics < max_medics && HAS_FLAG(mob.client.prefs.toggles_ert, PLAY_MEDIC) && check_timelock(mob.client, JOB_SQUAD_MEDIC, time_required_for_job))
		medics++
		to_chat(mob, SPAN_ROLE_HEADER("You are a Commando Medic of the Union of Progressive People, a powerful socialist state that rivals the United Americas!"))
		arm_equipment(mob, /datum/equipment_preset/upp/commando/medic, TRUE, TRUE)
	else
		to_chat(mob, SPAN_ROLE_HEADER("You are a Commando of the Union of Progressive People, a powerful socialist state that rivals the United Americas!"))
		arm_equipment(mob, /datum/equipment_preset/upp/commando, TRUE, TRUE)
