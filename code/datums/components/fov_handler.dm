...
/datum/component/fov_handler/proc/update_fov_size(mob/M, new_size)
	SIGNAL_HANDLER
	var/mob/parent_mob = parent
	var/client/parent_client = parent_mob.client
	if(!parent_client || !new_size) //Love client volatility!!
		return
...
/datum/component/fov_handler/proc/on_dir_change(mob/user, old_dir, new_dir)
	SIGNAL_HANDLER
...
/datum/component/fov_handler/RegisterWithParent()
	...
	RegisterSignal(parent, COMSIG_MOB_DEATH, PROC_REF(remove_mask))
	RegisterSignal(parent, COMSIG_HUMAN_REVIVED, PROC_REF(add_mask))
	RegisterSignal(parent, COMSIG_LIVING_REJUVENATED, PROC_REF(add_mask))
	RegisterSignal(parent, COMSIG_CLIENT_RESET_VIEW, PROC_REF(update_fov_size))
	RegisterSignal(parent, COMSIG_MOB_RESET_VIEW, PROC_REF(update_mask))
	RegisterSignal(parent, COMSIG_MOB_CHANGE_VIEW, PROC_REF(update_fov_size))
	RegisterSignal(parent, COMSIG_MOB_LOGIN, PROC_REF(update_mask))
	RegisterSignal(parent, COMSIG_MOB_LOGOUT, PROC_REF(mob_logout))

/datum/component/fov_handler/UnregisterFromParent()
	. = ..()
	UnregisterSignal(parent, list(COMSIG_MOB_RESET_VIEW, COMSIG_CLIENT_RESET_VIEW, COMSIG_MOB_CHANGE_VIEW, COMSIG_ATOM_DIR_CHANGE, COMSIG_MOB_DEATH, COMSIG_HUMAN_REVIVED))
