...
/obj/vehicle/train/MouseDrop_T(atom/movable/C, mob/user as mob)
	if(user.buckled || user.stat || user.is_mob_restrained() || !Adjacent(user) || !user.Adjacent(C) || !istype(C) || (user == C && !user.can_action))
		return
...
/obj/vehicle/train/verb/unlatch_v()
	...
	if(!usr.can_action || usr.is_mob_restrained() || !Adjacent(usr))
		return
