...
/datum/resin_construction/proc/can_build_here(turf/build_turf, mob/living/carbon/xenomorph/xeno)
	var/mob/living/carbon/xenomorph/blocker = locate() in build_turf
	if(blocker && blocker != xeno && blocker.stat != DEAD)
		to_chat(xeno, SPAN_WARNING("Can't do that with [blocker] in the way!"))
		return FALSE

	if(!istype(build_turf) || build_turf.weedable < FULLY_WEEDABLE || !can_xeno_build(build_turf))
		to_chat(xeno, SPAN_WARNING("You can't do that here."))
		return FALSE

	var/area/AR = get_area(build_turf)
	...
	var/obj/effect/alien/weeds/alien_weeds = locate() in build_turf
	if(!alien_weeds)
	...
	var/obj/vehicle/V = locate() in build_turf
	if(V)
		to_chat(xeno, SPAN_WARNING("You cannot build under \the [V]!"))
		return FALSE
	...
	if(istype(build_turf, /turf/closed/wall)) // Can't build in walls with no density
		to_chat(xeno, SPAN_WARNING("This area is too unstable to support a construction"))
		return FALSE

	if(!xeno.check_alien_construction(build_turf, check_doors = !can_build_on_doors))
		return FALSE

	if(range_between_constructions)
		for(var/i in urange(range_between_constructions, build_turf))
			var/atom/A = i
...
/datum/resin_construction/proc/build(turf/build_turf, datum/faction/faction, mob/living/carbon/xenomorph/builder)
	return

/datum/resin_construction/proc/check_thick_build(turf/build_turf, datum/faction/faction, mob/living/carbon/xenomorph/builder)
...
/datum/resin_construction/resin_obj/build(turf/build_turf, datum/faction/faction, mob/living/carbon/xenomorph/builder)
	var/path = check_thick_build(build_turf, builder, faction) ? build_path_thick : build_path
	if(pass_faction)
...
/datum/resin_construction/resin_turf/build(turf/build_turf, datum/faction/faction, mob/living/carbon/xenomorph/builder)
	var/path = check_thick_build(build_turf, builder, faction) ? build_path_thick : build_path

	build_turf.PlaceOnTop(path)

	var/turf/closed/wall/resin/resin_wall = build_turf
	if(istype(resin_wall) && pass_faction)
		resin_wall.faction = faction
		resin_wall.set_resin_builder(builder)
		set_hive_data(resin_wall, faction)
...
/datum/resin_construction/resin_obj/door/can_build_here(turf/build_turf, mob/living/carbon/xenomorph/xeno)
	if(!..())
		return FALSE

	var/wall_support = FALSE
	for(var/D in  GLOB.cardinals)
		var/turf/CT = get_step(build_turf, D)
		if(CT)
			if(CT.density)
				wall_support = TRUE
				break
...
/datum/resin_construction/resin_obj/nest/can_build_here(turf/build_turf, mob/living/carbon/xenomorph/xeno)
	if(!..())
		return FALSE

	var/obj/effect/alien/weeds/alien_weeds = locate() in build_turf // No need to check if null, because if there are no weeds then parent call fails any way
	if(!(alien_weeds.weed_strength >= WEED_LEVEL_HIVE))
		to_chat(xeno, SPAN_WARNING("These weeds are not strong enough to hold the nest."))
		return FALSE
...
/datum/resin_construction/resin_obj/acid_pillar/strong
	...
	build_path = /obj/effect/alien/resin/acid_pillar/strong
	build_time = 18 SECONDS

	range_between_constructions = 10
