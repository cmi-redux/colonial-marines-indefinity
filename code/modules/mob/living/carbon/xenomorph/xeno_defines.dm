...
/datum/caste_datum
	...
	var/special_structures_multiplier = 1
...
/client/proc/get_total_xeno_playtime(skip_cache = FALSE)
	...
	for(var/caste in SSticker.role_authority.castes_by_name)
...
/client/proc/get_total_t3_playtime()
	...
	for(var/caste_name in SSticker.role_authority.castes_by_name)
		caste = SSticker.role_authority.castes_by_name[caste_name]
		if(caste.tier < 3)
			continue
