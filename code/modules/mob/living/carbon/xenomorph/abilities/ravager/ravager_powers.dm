...
/datum/action/xeno_action/activable/pounce/charge/additional_effects(mob/living/living)
	...
	if(xeno.mutation_type != RAVAGER_NORMAL)
		return
	var/datum/behavior_delegate/ravager_base/ravager_base = xeno.behavior_delegate
	if(ravager_base.empower_targets < ravager_base.super_empower_threshold)
		return
	...
	human.apply_effect(ravager_base.knockdown_amount, WEAKEN)
	...
	for(var/x in 0 to ravager_base.fling_distance-1)
		temp = get_step(turf, facing)
		if(!temp)
			break
		turf = temp

	human.throw_atom(human, ravager_base.fling_distance, SPEED_VERY_FAST, xeno, TRUE)
...
/datum/action/xeno_action/activable/clothesline/use_ability(atom/A)
	...
	if(!isxeno_human(A) || xeno.can_not_harm(A))
	...
	H.throw_atom(T, fling_distance, SPEED_VERY_FAST, xeno, TRUE)
...
/datum/action/xeno_action/onclick/spike_shield/use_ability(atom/target)
	...
	if(xeno.mutation_type == RAVAGER_HEDGEHOG)
		var/datum/behavior_delegate/ravager_hedgehog/BD = xeno.behavior_delegate
		if(!BD.check_shards(shard_cost))
			to_chat(xeno, SPAN_DANGER("Not enough shards! You need [shard_cost - BD.shards] more!"))
			return
...
/datum/action/xeno_action/activable/rav_spikes/use_ability(atom/A)
	...
	xeno.visible_message(SPAN_XENOWARNING("The [xeno] fires their spikes at [A]!"), SPAN_XENOWARNING("You fire your spikes at [A]!"))
