...
/mob/living/proc/in_fov(atom/observed_atom, ignore_self = FALSE)
	...
	if(fov_view && observed_atom.plane == GAME_PLANE_FOV_HIDDEN)
	...
	if(. && (eye_blind || blinded || eye_blurry))
...
/mob/living/proc/update_fov()
	var/highest_fov
	if(CONFIG_GET(flag/native_fov) && !isxeno(src) && !issynth(src))
...
/proc/play_fov_effect(atom/center, range, icon_state, dir = SOUTH, ignore_self = FALSE, angle = 0, list/override_list)
	...
	for(var/mob/living/living_mob in override_list || get_mobs_in_view(range, center))
		var/client/mob_client = living_mob.client
		if(!mob_client)
			continue
		if(living_mob.ear_deaf) //Deaf people can't hear sounds so no sound indicators
			continue
