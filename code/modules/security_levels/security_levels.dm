		...
		switch(level)
			...
			if(SEC_LEVEL_DELTA)
				if(announce)
					...
					var/input = "ОПАСНОСТЬ, СИСТЕМА АВАРИЙНОГО САМОУНИЧТОЖЕНИЯ АКТИВИРОВАНА. ПРОЙДИТЕ В КАМЕРУ САМОУНИЧТОЖЕНИЯ ДЛЯ ВВЕДЕНИЯ УПРАВЛЯЮЩЕГО СТЕРЖНЯ."
					faction_announcement(input, name, 'sound/AI/selfdestruct_short.ogg')
