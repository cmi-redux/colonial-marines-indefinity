...
/obj/item/weapon/gun/proc/unload_chamber(mob/user)
	if(!in_chamber)
		return
	var/found_handful
	var/obj/projectile/chambered = in_chamber
...
/obj/item/weapon/gun/proc/load_into_chamber(mob/user)
	...
	if(active_attachable)
		...
		if(active_attachable.ammo_position > 0) //If it's still got ammo and stuff.
			var/obj/projectile/proj = active_attachable.current_rounds[active_attachable.ammo_position]
...
/obj/item/weapon/gun/proc/apply_traits(obj/projectile/proj)
...
/obj/item/weapon/gun/proc/ready_in_chamber()
	if(current_mag && current_mag.ammo_position > 0)
		var/obj/projectile/proj = current_mag.current_rounds[current_mag.ammo_position]
...
/obj/item/weapon/gun/proc/Fire(atom/target, mob/living/user, params, atom/operator, reflex = FALSE, dual_wield)
	set waitfor = FALSE
...
/obj/item/weapon/gun/proc/handle_fire(atom/target, mob/living/user, params, reflex = FALSE, dual_wield, check_for_attachment_fire, akimbo, fired_by_akimbo)
	...
	if(targloc != curloc || flags_mounted_gun_features & GUN_MOUNTING)
		...
		INVOKE_ASYNC(projectile_to_fire, TYPE_PROC_REF(/obj/projectile, fire_at), target, user, src, null, bullet_velocity)
	...
	for(bullets_fired = 1 to bullets_to_fire)
		...
		if(projectile_to_fire.ammo.bonus_projectiles_amount)
			...
			for(var/i in 1 to projectile_to_fire.ammo.bonus_projectiles_amount)
				proj = new /obj/projectile(attacked_mob.loc, create_cause_data(initial(name), user))
		...
		if(active_attachable)
			if(active_attachable.flags_attach_features & ATTACH_PROJECTILE)
				if(!(active_attachable.flags_attach_features & ATTACH_WIELD_OVERRIDE) && !(flags_item & WIELDED))
					...
					return
		if((((flags_gun_features & GUN_WIELDED_FIRING_ONLY) && !(flags_item & WIELDED) && !flags_mounted_gun_features) || (flags_mounted_gun_features & GUN_CAN_OVERRIDE_MOUNTED && !(flags_mounted_gun_features & GUN_MOUNTED))) && !active_attachable) //If we're not holding the weapon with both hands when we should.
