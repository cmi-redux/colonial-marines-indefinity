...
// Check zlevel trait for minimap for comapere minimap by trait, returning trait
/datum/controller/subsystem/mapping/proc/level_minimap_trait(level)
	for(var/trait in TCMP_MAPS_TRAITS)
		if(level_trait(level, trait))
			return trait
