...
/obj/structure/machinery/power/smes/ui_act(action, list/params)
	. = ..()
	if(.)
		return
	switch(action)
...
/obj/structure/machinery/power/smes/proc/ion_act()
	if(is_ground_level(z))
		if(prob(1)) //explosion
			...
			smoke.attach(src)
			smoke.start()
			cell_explosion(get_turf(src), 300, 50, EXPLOSION_FALLOFF_SHAPE_LINEAR, null, create_cause_data("взрыва смеса", usr))
			qdel(src)
			return
