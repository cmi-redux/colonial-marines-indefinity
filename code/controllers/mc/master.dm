...
/datum/controller/master/proc/init_subsystem(datum/controller/subsystem/subsystem)
	...
	switch(result)
		if(SS_INIT_FAILURE)
			message_prefix = "Не удалось инициализировать [subsystem.name] подсистему после"
			chat_warning = TRUE
		if(SS_INIT_SUCCESS)
			message_prefix = "Инициализировано [subsystem.name] подсистема за"
		if(SS_INIT_NO_NEED)
			// This SS is disabled or is otherwise shy.
			return
		else
			// SS_INIT_NONE or an invalid value.
			message_prefix = "Инициализировано [subsystem.name] подсистема с ошибками за"
			chat_warning = TRUE
