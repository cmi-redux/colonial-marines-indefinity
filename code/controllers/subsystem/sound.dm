...
/datum/controller/subsystem/sound/proc/queue(datum/sound_template/template, list/client/hearers, list/datum/interior/extra_interiors)
	...
	if(extra_interiors && SSmapping)
		for(var/datum/interior/interior in extra_interiors)
			if(interior?.ready)
				var/list/bounds = interior.get_middle_coords()
				if(bounds.len >= 2)
					hearers |= SSquadtree.players_in_range(RECT(bounds[1], bounds[2], interior.map_template.height, interior.map_template.width), bounds[3])
