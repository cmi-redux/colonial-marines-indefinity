...
/datum/controller/subsystem/sound/proc/queue(datum/sound_template/template, list/client/hearers, list/datum/interior/extra_interiors)
	...
	if(extra_interiors && SSmapping)
		for(var/datum/interior/vehicle_interior in extra_interiors)
			if(vehicle_interior?.ready)
				var/list/bounds = vehicle_interior.get_middle_coords()
				if(bounds.len >= 2)
					hearers |= SSquadtree.players_in_range(RECT(bounds[1], bounds[2], vehicle_interior.map_template.width, vehicle_interior.map_template.height), bounds[3])
