...
/mob/living/carbon/human/MouseDrop_T(atom/dropping, mob/user)
	...
	if(isxeno(dropping))
		...
		if(xenomorph.back)
			...
			if(!do_after(user, HUMAN_STRIP_DELAY * user.get_skill_duration_multiplier(SKILL_CQC), INTERRUPT_ALL, BUSY_ICON_GENERIC, xenomorph, INTERRUPT_MOVED, BUSY_ICON_GENERIC))
