 ...
 */
/proc/playsound(atom/source, soundin, vol = 100, vary = FALSE, sound_range, vol_cat = VOLUME_SFX, channel = 0, status, falloff = 1, list/echo, y_s_offset, x_s_offset, zrange = 1)
	if(isarea(source))
	...
	if(SSinterior.in_interior(turf_source))
		var/datum/interior/interior = SSinterior.get_interior_by_coords(turf_source.x, turf_source.y, turf_source.z)
		if(interior?.ready)
			extra_interiors |= interior
			if(interior.exterior)
				var/turf/new_turf_source = get_turf(interior.exterior)
...
/proc/get_sfx(sound)
	if(istext(sound))
		switch(sound)
			...
			// Human
			if("male_scream")
				sound = pick('sound/voice/human_male_scream_1.ogg','sound/voice/human_male_scream_2.ogg','sound/voice/human_male_scream_3.ogg','sound/voice/human_male_scream_4.ogg',5;'sound/voice/human_male_scream_5.ogg',5;'sound/voice/human_jackson_scream.ogg',5;'sound/voice/human_ack_scream.ogg')
			if("male_pain")
...
/client/proc/sound_debug_query()
	...
	for(var/sound/sound in SoundQuery())
		UNLINT(to_chat(src, "channel#[sound.channel]: [sound.status] - [sound.file] - len=[sound.len], wait=[sound.wait], offset=[sound.offset], repeat=[sound.repeat]")) // unlint until spacemandmm suite-1.7
