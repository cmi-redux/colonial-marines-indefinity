...
//RECORD DELETE
			...
			if("Add Entry")
				...
				if((!t1 || usr.stat || usr.is_mob_restrained() || (!in_range(src, usr) && (!ishighersilicon(usr))) || active2 != a2))
					return
				var/created_at = text("[]&nbsp;&nbsp;[]&nbsp;&nbsp;[]", time2text(world.realtime, "MMM DD"), time2text(world.time, "[game_time_timestamp()]:ss"), GLOB.game_year)
			...
			if("Delete Entry")
				...
				if(active2.fields["comments"][href_list["del_c"]])
					...
					updated_comments[href_list["del_c"]]["deleted_at"] = text("[]&nbsp;&nbsp;[]&nbsp;&nbsp;[]", time2text(world.realtime, "MMM DD"), time2text(world.time, "[game_time_timestamp()]:ss"), GLOB.game_year)
