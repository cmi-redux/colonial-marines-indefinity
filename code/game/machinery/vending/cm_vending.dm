...
/obj/structure/machinery/cm_vending/sorted
	name = "ColMarTech generic sorted rack/vending_machine"
...
/obj/structure/machinery/cm_vending/own_points
	name = "ColMarTech generic vending_machine"
	...
	if(length(itemspec)) //making sure it's not empty
		...
		if(islist(prod_type))
			for(var/each_type in prod_type)
				vendor_successful_vend_one(vending_machine, each_type, user, target_turf, itemspec[4] == VENDOR_CAN_BUY_UNIFORM)
		else
			vendor_successful_vend_one(vending_machine, prod_type, user, target_turf, itemspec[4] == VENDOR_CAN_BUY_UNIFORM)
