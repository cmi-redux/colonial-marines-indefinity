...
/obj/item/stack/Topic(href, href_list)
	...
	if(href_list["make"])
		...
		if((R.stack_flags & RESULT_REQUIRES_SNOW) && !(istype(usr.loc, /turf/open/snow) || istype(usr.loc, /turf/open/auto_turf/snow)))
...
/datum/stack_recipe
	...
	var/stack_flags = NO_FLAGS

/datum/stack_recipe/New(title, result_type, req_amount = 1, res_amount = 1, max_res_amount = 1, time = 0, one_per_turf = 0, on_floor = 0, skill_req, skill_lvl = 0, min_time = 0, stack_flags = NO_FLAGS)
	src.title = title
